...
class OnnxRuntime(Runtime):
    ...
    #         y += self._session.run(None, {self._inputName: input[:,i,:].numpy()})
    #
    #     y = np.reshape(np.vstack([y[i][0, :, :] for i in range(len(y))]), [1, len(y) * y[0].shape[1], y[0].shape[2]])
    ...
    def _runOnnxruntimeSession(self, input: torch.Tensor) -> _DecodeResult:
        ...
        return _DecodeResult(label_sequences=[[label_sequences]], scores=[scores], timesteps=timesteps)
