...
class W2lDecoder(object):
    def __init__(self, options, tgt_dict):
        self.tgt_dict = tgt_dict
        self.vocab_size = len(tgt_dict)
        self.nbest = options["nbest"]
...
class W2lViterbiDecoder(W2lDecoder):
    def __init__(self, options, tgt_dict):
        super().__init__(options, tgt_dict)
...
class W2lKenLMDecoder(W2lDecoder):
    def __init__(self, options, tgt_dict):
        ...
        self.unit_lm = getattr(options, "unit_lm", False)

        if options["lexicon"]:
            self.lexicon = load_words(options["lexicon"])
            self.word_dict = create_word_dict(self.lexicon)
            self.unk_word = self.word_dict.get_index("<unk>")

            self.lm = KenLM(options["kenlm_model"], self.word_dict)
            ...
            self.decoder_opts = LexiconDecoderOptions(
                ...
                beam_size_token=int(getattr(options, "beam_size_token", len(tgt_dict))),
                beam_threshold=options["beam_threshold"],
                lm_weight=options["lm_weight"],
                word_score=options["word_score"],
                unk_score=options["unk_weight"],
                sil_score=options["sil_weight"],
                log_add=False,
            ...
            assert options["unit_lm"], "lexicon free decoding can only be done with a unit language model"
...
class W2lFairseqLMDecoder(W2lDecoder):
    def __init__(self, options, tgt_dict):
        ...
        self.lexicon = load_words(options["lexicon"]) if options["lexicon"] else None
        self.idx_to_wrd = {}

        checkpoint = torch.load(options["kenlm_model"], map_location="cpu")
        ...
        with open_dict(lm_args.task):
            lm_args.task.data = osp.dirname(options["kenlm_model"])
