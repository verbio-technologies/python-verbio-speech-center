...
def retrieveToken(options):
    if options.client_id:
        return _get_refreshed_token(options.client_id, options.client_secret, options.token_file)
    else:
        return _read_token(token_file=options.token_file)

def _read_token(token_file: str) -> str:
    with open(token_file) as token_hdl:
        return ''.join(token_hdl.read().splitlines())

def _get_refreshed_token(client_id, client_secret, token_file):
    currentToken = _read_token(token_file=token_file)
    refresh = False
    try:
        ...
        refresh = True
    if refresh:
        newToken = _refresh_token(client_id, client_secret)
        _writeNewToken(newToken, token_file)
        return newToken
    else:
