...
then
	...
	expected_wer=$(jq --arg keyvar "$language-$variant" '.[$keyvar].oov' "tests/e2e/data/expected_metrics.json")
	expected_oov=$(jq --arg keyvar "$language-$variant" '.[$keyvar].wer' "tests/e2e/data/expected_metrics.json")
else

	expected_wer=$(jq --arg keyvar "$language" '.[$keyvar].oov' "tests/e2e/data/expected_metrics.json")
	expected_oov=$(jq --arg keyvar "$language" '.[$keyvar].wer' "tests/e2e/data/expected_metrics.json")
fi
