...
from bin.server import setDefaultBindAddress, TomlConfigurationOverride, SystemVarsOverride, checkArgsRequired
...
class TestTomlConfigurationOverride(unittest.TestCase):
    ...
    def test_TomlConfigurationOverride(self, mock_toml_load, mock_os_path_exists):
        ...
        mock_toml_load.return_value = {'global': {'host': '127.0.0.1', 'port': 8080},
        ...
        mock_os_path_exists.assert_called_once_with(self.args.config or "asr4_config.toml")
        mock_toml_load.assert_called_once_with(self.args.config or "asr4_config.toml")

    @patch('os.path.exists')
    @patch('toml.load')
    def test_TomlConfigurationOverride_with_default_config_file(self, mock_toml_load, mock_os_path_exists):
