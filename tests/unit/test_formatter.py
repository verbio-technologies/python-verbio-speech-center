...
class TestTimestampManagement(unittest.TestCase):
    def testSingleWord(self):
        ...
        wordTimestamps = [(0.0,0.15)]
        wordFrames = [[1,4,8]]
        changeKO = json.loads("{\"operations\": [{\"Merge\": [{\"start\": 0, \"end\": 1}, \"hi-fi\"]}]}")["operations"]
    ...
    def testTwoWords(self):
        ...
        changeOK = json.loads("{\"operations\": [{\"Merge\": [{\"start\": 0, \"end\": 1}, \"pink-floyd\"]}]}")["operations"]
        (times, frames) = TimeFixer(changeOK, wordTimestamps, wordFrames).invoke()
        ...
        wordTimestamps = [(0.0,0.15),(0.18,0.28)]
        wordFrames = [[1,4], [10,16,22]]
        changeKO = json.loads("{\"operations\": [{\"Merge\": [{\"start\": 2, \"end\": 5}, \"pink-floyd\"]}]}")["operations"]
        (times, frames) = TimeFixer(changeKO, wordTimestamps, wordFrames).invoke()
        self.assertEqual(times, wordTimestamps)
        self.assertEqual(frames, wordFrames)
    ...
    def testManyWords(self):
        ...
        change1 = json.loads("{\"operations\": [{\"Merge\": [{\"start\": 1, \"end\": 2}, \"33\"]}, {\"Change\": [1, \"33,\"]}]}")["operations"]
        ...
        wordTimestamps = [(0.0,0.15),(0.18,0.28),(0.30,0.33),(0.39,0.43)]
        wordFrames = [[1,4], [10,16,22], [25,29], [30,31,32,33,34]]
        change2 = json.loads("{\"operations\": [{\"Merge\": [{\"start\": 0, \"end\": 3}, \"****\"]}]}")["operations"]
        (times, frames) = TimeFixer(change2, wordTimestamps, wordFrames).invoke()
        self.assertEqual(times, [(0.0,0.43)])
        self.assertEqual(frames, [[1,4,10,16,22,25,29,30,31,32,33,34]])

    def testReal(self):
        text = ["what", "is", "the", "cost", "of", "flight", "cee", "o", "one", "six", "three", "one", "from", "nashville", "to", "chicago"]
        wordFrames=[[18, 19, 21, 22], [27, 29], [33, 34, 35], [40, 44, 49, 52], [58, 59], [67, 71, 75, 77, 79, 81], [100, 104, 108], [121], [155, 158, 159], [168, 172, 176], [192, 193, 196, 198, 202], [212, 215, 217], [229, 231, 233, 235], [242, 245, 250, 251, 255, 257, 259, 261, 262], [268, 271], [276, 277, 280, 284, 288, 295, 297]]
        wordTimestamps=[(0.36, 0.44), (0.54, 0.58), (0.66, 0.7000000000000001), (0.8, 1.04), (1.16, 1.18), (1.34, 1.62), (2.0, 2.16), (2.42, 2.42), (3.1, 3.18), (3.36, 3.52), (3.84, 4.04), (4.24, 4.34), (4.58, 4.7), (4.84, 5.24), (5.36, 5.42), (5.5200000000000005, 5.94)]
        ...
        (times, frames) = TimeFixer(ops["operations"], wordTimestamps, wordFrames).invoke()

        self.assertEqual(times, [(0.36, 0.44), (0.54, 0.58), (0.66, 0.7000000000000001), (0.8, 4.04), (4.24, 4.34), (4.58, 4.7), (4.84, 5.24), (5.36, 5.42), (5.5200000000000005, 5.94)] )
        self.assertEqual(frames, [[18, 19, 21, 22], [27, 29], [33, 34, 35], [40, 44, 49, 52, 58, 59, 67, 71, 75, 77, 79, 81, 100, 104, 108, 121, 155, 158, 159, 168, 172, 176, 192, 193, 196, 198, 202], [212, 215, 217], [229, 231, 233, 235], [242, 245, 250, 251, 255, 257, 259, 261, 262], [268, 271], [276, 277, 280, 284, 288, 295, 297]] )
