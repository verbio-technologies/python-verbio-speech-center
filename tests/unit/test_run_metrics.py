...
class MockMetrics(Metrics):
	...
	def __init__(self):
		...
		obtained_intratest_folder = os.path.join(self.data_parent,"data/test_es_intratest/")
		expected_metrics = os.path.join(self.data_parent,"data/expected_metrics.json")
		test_type = "basic"
		language = "es"
		super().__init__(obtained_accuracy, obtained_oov, obtained_intratest_folder, expected_metrics, test_type, language)
...
class TestGetMetrics(unittest.TestCase):
	...
	def testMakeIntratestComparison(self):
		metrics = MockMetrics()
		reference_data = json.load(open(metrics.expected_metrics))[metrics.language][metrics.test_type]
		metrics.make_intratest_comparison(reference_data, "dialects")
		self.assertEqual(metrics.TEST_PASSED, True)
		
	def testMakeIntratestComparisonLowModelAccuracy(self):
		metrics = MockMetrics()
		reference_data = {'domains_typical_deviation': 5.15, 'domains': {'multidomain-real_estate-hh-servihabitat': {'accuracy': 100}}}
		metrics.make_intratest_comparison(reference_data, "domains")
		self.assertEqual(metrics.TEST_PASSED, False)

	def testMakeIntratestComparisonMissingData(self):
		metrics = MockMetrics()
		reference_data = {'domains_typical_deviation': 5.15, 'domains': {'domain-only-in-reference': {'accuracy': 0}}}
		metrics.make_intratest_comparison(reference_data, "domains")
		self.assertEqual(metrics.TEST_PASSED, False)